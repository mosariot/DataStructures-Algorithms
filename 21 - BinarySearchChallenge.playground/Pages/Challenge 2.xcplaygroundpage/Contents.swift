...
*/
func findIndicesArrayBased(of value: Int, in array: [Int]) -> Range<Int>? {
    guard let leftIndex = array.firstIndex(of: value) else { return nil }
    guard let rightIndex = array.lastIndex(of: value) else { return nil }
...
func startIndex(of value: Int, in array: [Int], range: CountableRange<Int>) -> Int? {
    ...
    if array[middleIndex] == value {
        if array[middleIndex - 1] != value { return middleIndex }
        else { return startIndex(of: value, in: array, range: range.lowerBound..<middleIndex) }
...
func endIndex(of value: Int, in array: [Int], range: CountableRange<Int>) -> Int? {
    ...
    if middleIndex == 0 || middleIndex == array.count - 1 {
        if array[middleIndex] == value { return middleIndex + 1 }
        else { return nil }
    }
    
    if array[middleIndex] == value {
        if array[middleIndex + 1] != value { return middleIndex + 1 }
        else { return endIndex(of: value, in: array, range: middleIndex..<range.upperBound) }
    } else if value < array[middleIndex] {
...
let array = [1, 2, 3, 3, 3, 4, 5, 5]
if let ind = findIndicesArrayBased(of: 3, in: array) {
    print(ind)
}
if let ind = findIndicesBinarySearchBased(of: 3, in: array) {
    print(ind)
}
